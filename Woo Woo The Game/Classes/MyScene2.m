...
- (id)init
{
    ...
    CCButton *pause = [CCButton buttonWithTitle:@"" spriteFrame:[CCSpriteFrame frameWithImageNamed:@"btn_pausa.png"] highlightedSpriteFrame:[CCSpriteFrame frameWithImageNamed:@"btn_pausa.png"] disabledSpriteFrame:[CCSpriteFrame frameWithImageNamed:@"btn_pausa.png"]];    pause.positionType = CCPositionTypeNormalized;
    //[sound setScale:0.5];
...
*/
    ...
    //streak = [CCMotionStreak streakWithFade:0.3f minSeg:5 width:8 color:[CCColor colorWithUIColor:[UIColor whiteColor]] textureFilename:@"spada.png"];
    
    streak = [CCMotionStreak streakWithFade:0.3 minSeg:20 width:6 color:[CCColor colorWithUIColor:[UIColor whiteColor]] textureFilename:@"spada.png"];
...
-(void) touchBegan:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    for (GabbianoSprite *sprite in movableSprites) {
        
        float distance = pow(sprite.position.x - touchLoc.x, 2) + pow(sprite.position.y - touchLoc.y, 2);
...
-(void)touchBegan:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    streak.position = touchLoc;
    [self addChild:streak z:2];
    
    int rimossi = 0;
    
    for (int i = 0 ; i < [movableSprites count] - rimossi; i++) {
        
        GabbianoSprite *sprite = [movableSprites objectAtIndex:i];
        ...
        if (distance <= 50) {
            ...
            if([sprite returnAmico] == [NSNumber numberWithInt:1]) {
                punteggio++;
                if([singleton is_sound])
                    [[OALSimpleAudio sharedInstance] playEffect:@"bird_00.mp3" loop:NO];
            } else if([sprite returnBonus] == [NSNumber numberWithInt:1]) {
                for(int i = 0 ; i < 3 ; i++) {
                    if(colpi_mostro != 0) {
                        colpi_mostro--;
                        [self controlla_vita];
...
-(void)touchMoved:(UITouch *)touch withEvent:(UIEvent *)eventy  {
...
- (void)updateWithTimeSinceLastUpdate:(CFTimeInterval)timeSinceLast {
    ...
    if (self.lastSpawnTimeInterval > 5.2) {
        self.lastSpawnTimeInterval = 4.5;
        
        if(mostri == 0)
            [self addAmico];
        else if(mostri == 17 || mostri == 34 || mostri == 51 || mostri == 68 || mostri == 81 || mostri == 98 || mostri == 115 || mostri == 132) {
            [self addBonus];
        }
        else if(mostri % 5)
            [self addAmico];
...
- (void)addBonus {
    ...
    CCActionFlipX *revese = [CCActionFlipY actionWithFlipY:true];
    ...
    [monster setBonus:[NSNumber numberWithInt:1]];
    ...
    float increment_inverse_velocity = 360.0f;
    ...
    [movableSprites addObject:monster];
...
- (void)addAmico {
    ...
    // Create sprite
    GabbianoSprite *monster = [GabbianoSprite GabbianoSpriteAmico];
    
    [monster setAmico:[NSNumber numberWithInt:1]];
...
- (void)addMonster {
    ...
    [monster setAmico:[NSNumber numberWithInt:0]];
    ...
    [movimenti addObject:[CCActionFadeOut actionWithDuration:0.5]];
    [movimenti addObject:[CCActionRemove action]];
...
-(void)game_over {
    ...
    [[CCDirector sharedDirector] replaceScene:[GameOverSceneMamma scene]
...
-(void)checkPause {
    ...
    riprendi.position = ccp(0.50f, 0.35f);
